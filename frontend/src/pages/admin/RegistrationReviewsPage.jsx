...
export default function RegistrationReviewsPage() {
  ...
  const [state, setState] = useState({ status: "loading", registrations: [], error: "" });
  useEffect(() => { let active = true; adminService.getRegistrations({ status, type: type === "all" ? undefined : type }).then((data) => active && setState({ status: "success", registrations: data.registrations, error: "" })).catch((error) => active && setState({ status: "error", registrations: [], error: getApiError(error).message })); return () => { active = false; }; }, [type, status, retry]);
  function choose(setter, value) { setter(value); setState((current) => ({ ...current, status: "loading" })); }
